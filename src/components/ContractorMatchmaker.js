...
export default function ContractorMatchmaker({ zip, category }) {
  ...
  useEffect(() => {
    ...
    const fetchContractors = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/v1/contractors/match?zip=${zip}&category=${category}`);
  ...
  if (contractors.length === 0) {
    return (
      <div className="p-6 text-center">
        ...
        <p className="mt-4 text-sm text-gray-500">No contractors found matching your criteria</p>
      </div>
    );
  }
              ...
              <div className="flex items-center space-x-3 mb-2">
                ...
                <button 
                  onClick={() => {
                    ...
                    alert(`Requesting quote from ${contractor.Name || 'this contractor'}...`);
                  }}
